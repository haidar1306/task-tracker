...
{
    /**
     * Keep this table's pagination state separate from other paginated views.
     */
    ...
    public string $emptyMessage = 'Not Found';
        ...
        return $query
            ->when($this->getFilter('search'), function ($query, $term) {
                return $query
                    ->search($term)
                    ->orderByRaw(
                        'CASE WHEN name LIKE ? THEN 0 WHEN email LIKE ? THEN 1 ELSE 2 END',
                        [$term.'%', $term.'%']
                    );
            })
    ...
    public function highlightSearch(?string $value): string
    {
        $value = e((string) $value);
        $term = trim((string) data_get($this->filters, 'search'));
